...
/** The original homepage layout -- teams grouped by conference, then
 * division. Extracted unchanged out of page.tsx so HomepageViewToggle can
 * switch between this and TierGrid. */
export function DivisionGrid({ teams, season }: { teams: HomepageTeam[]; season: number }) {
  const byDivision = groupByDivision(teams);

  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      {CONFERENCES.map((conf) => (
        <div key={conf.key} className="rounded-2xl border-2 p-5 sm:p-6" style={{ borderColor: conf.accent }}>
